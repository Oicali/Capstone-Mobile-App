...
function DatePickerBtn({ label, value, onChange, maximumDate }) {
  ...
  if (Platform.OS === "ios")
    return (
      <View style={{ flex: 1 }}>
        ...
        >
          <Text style={[dp.btnTxt, !value && { color: G400 }]}>
            {disp || "Select date"}
          </Text>
              ...
              <View style={dp.iosSh}>
                <View style={dp.iosHdr}>
                  <TouchableOpacity onPress={() => setShow(false)}>
                    <Text style={dp.iosCan}>Cancel</Text>
                  </TouchableOpacity>
                  ...
                  >
                    <Text style={dp.iosDone}>Done</Text>
                  </TouchableOpacity>
                </View>
                <DateTimePicker
                  value={temp}
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {show && (
        <DateTimePicker
          value={value ? isoDate(value) : new Date()}
          mode="date"
          display="calendar"
          onChange={(e, d) => {
            setShow(false);
      ...
      <View style={s.pgBtns}>
        ...
        {tp > 1 &&
          Array.from({ length: Math.min(tp, 5) }, (_, i) => (
            <TouchableOpacity
              key={i}
              style={[s.pgBtn, page === i && s.pgAct]}
              onPress={() => onPage(i)}
            >
              <Text style={[s.pgTxt, page === i && { color: WHITE }]}>
                {i + 1}
              </Text>
            </TouchableOpacity>
          ))}
        <TouchableOpacity
          style={[s.pgBtn, page === tp - 1 && s.pgDis]}
          onPress={onNext}
          disabled={page === tp - 1}
        >
          <Ionicons
        ...
        <View style={s.detailSh}>
          ...
          >
            <View style={s.rankCircle}>
              <Text style={s.rankTxt}>#{data.rank}</Text>
            </View>
          ...
          <View style={s.detailRow}>
            ...
            {data.type && (
              <View style={s.detailStat}>
                <Text style={[s.detailVal, { fontSize: 14 }]} numberOfLines={2}>
                  {data.type}
                </Text>
      ...
      <View style={s.msOv}>
        <View style={s.msSh}>
            ...
            <View style={s.msSearch}>
              ...
              {!!search && (
                <TouchableOpacity onPress={() => setSearch("")}>
                  <Ionicons name="close-circle" size={16} color={G400} />
                </TouchableOpacity>
              )}
            </View>
          )}
          <View style={s.msActions}>
            <TouchableOpacity
              onPress={() =>
                setDraft(draft.length === items.length ? [] : [...items])
              }
            >
              <Text style={s.msActTxt}>
                {draft.length === items.length ? "Clear all" : "Select all"}
              </Text>
            </TouchableOpacity>
            {draft.length > 0 && (
              <TouchableOpacity
                ...
                onPress={() => setDraft([])}
              >
                <Text style={[s.msActTxt, { color: RED }]}>
                  Clear ({draft.length})
                </Text>
              </TouchableOpacity>
            )}
            ...
            {filtLeg.length > 0 && (
              <>
                <View style={s.msGroup}>
                  <View style={s.msGroupLine} />
                  <Text style={s.msGroupTxt}>
          ...
          <View style={s.msFoot}>
            <TouchableOpacity style={s.msCancel} onPress={onClose}>
              <Text style={s.msCancelTxt}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.msApply}
              onPress={() => {
...
function FilterSheet({ visible, applied, onApply, onClose }) {
  ...
  const handlePreset = (key) => {
    ...
    if (r)
      setDraft((f) => ({ ...f, preset: key, dateFrom: r.from, dateTo: r.to }));
    setDateErr("");
  };
  const onFrom = (d) => {
    const iso = fmtISO(d);
    setDraft((f) => ({ ...f, dateFrom: iso }));
    setDateErr(valDates(iso, draft.dateTo));
  };
  const onTo = (d) => {
    const iso = fmtISO(d);
    setDraft((f) => ({ ...f, dateTo: iso }));
    setDateErr(valDates(draft.dateFrom, iso));
  };
  const doApply = () => {
    if (draft.preset === "custom") {
      const e = valDates(draft.dateFrom, draft.dateTo);
      if (e) {
        setDateErr(e);
        return;
      }
          ...
          <View style={s.fsSh}>
            ...
            <View style={s.fsTop}>
              ...
              >
                ...
                {dirty && (
                  <View style={s.fsDirty}>
                    <Text style={s.fsDirtyTxt}>modified</Text>
                  </View>
                )}
              </View>
              <TouchableOpacity
              ...
              >
                {PRESETS.map((p) => (
                  <TouchableOpacity
                    ...
                    style={[
                      s.presetChip,
                      draft.preset === p.key && s.presetChipOn,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        s.presetTxt,
                        draft.preset === p.key && s.presetTxtOn,
                      ]}
                    >
                      {p.label}
                    </Text>
                ...
                <View style={{ paddingHorizontal: 20, marginTop: 12 }}>
                  ...
                  >
                    ...
                    <View style={{ paddingTop: 30, paddingHorizontal: 2 }}>
                      <Text style={{ color: G400, fontSize: 16 }}>→</Text>
                    </View>
                    <DatePickerBtn
                      label="End Date"
                      value={draft.dateTo}
                ...
                >
                  <View style={s.dropInner}>
                    {draft.crimeTypes.length === 0 ? (
                      <Text style={s.dropPh}>All Crimes</Text>
                    ) : (
                      <>
                          ...
                          <View key={c} style={s.pill}>
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                setDraft((f) => ({
                                  ...f,
                                  crimeTypes: f.crimeTypes.filter(
                                    (x) => x !== c,
                                  ),
                                }))
                              }
      ...
      {cards.map((c, i) => (
        ...
        >
          ...
          <View style={s.statTopRow}>
            ...
            <Text style={s.statSubTxt} numberOfLines={2}>
              {c.sub}
            </Text>
          </View>
    ...
    <View style={s.card}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View>
          ...
          <View style={s.tFoot}>
            <Text
              style={[s.tD, { width: 112, fontWeight: "700", color: NAVY }]}
            >
              TOTAL
            </Text>
            <Text
  ...
  if (!data || data.length === 0)
    return (
      <View style={s.card}>
        ...
        >
          <Text style={{ fontSize: 12, color: G600 }}>No trend data</Text>
        </View>
      </View>
    );
...
function CrimeClock({ data }) {
  ...
  return (
    <View style={s.card}>
      <View style={s.cardHdr}>
        <Text style={s.cardTitle}>Crime Clock — Hourly Distribution</Text>
            ...
            >
              ...
              >
                {r.count > 0 && (
                  <View
                    style={{
                      width: Math.max((r.count / maxV) * (SW - 224), 4),
                      height: "100%",
                      backgroundColor: i % 2 === 0 ? NAVY : NAVY_M,
                      borderRadius: 4,
                    }}
                  />
                )}
...
function PlaceTable({ data }) {
  ...
  const [detail, setDetail] = useState(null);
  const sorted = [...data]
    .sort((a, b) => (dir === "desc" ? b.count - a.count : a.count - b.count))
    .map((d, i) => ({ ...d, rank: i + 1 }));
    ...
    <>
      <View style={s.card}>
        <View style={s.cardHdr}>
          ...
          <TouchableOpacity
            onPress={() => setDir((d) => (d === "desc" ? "asc" : "desc"))}
            style={{ flexDirection: "row", alignItems: "center", gap: 3 }}
          >
            <Text style={{ fontSize: 10, color: G600, fontWeight: "600" }}>
              Count
            </Text>
            <Ionicons
        ...
        {pg.map((r, i) => (
          ...
          >
            ...
            <Text
              style={[
                s.tD,
                { flex: 1, color: NAVY_M, fontWeight: "600", fontSize: 11 },
              ]}
              numberOfLines={2}
            >
              {r.place}
            ...
            <View style={[s.tD, { width: 22, alignItems: "center" }]}>
              <Ionicons name="chevron-forward" size={11} color={G400} />
            </View>
          </TouchableOpacity>
        ))}
        <Pgn
...
function BarangayTable({ data }) {
  ...
  const SBtn = ({ c, l, sty }) => (
    <TouchableOpacity
      style={[{ flexDirection: "row", alignItems: "center", gap: 2 }, sty]}
      onPress={() => sort(c)}
    >
      <Text style={s.tHTxt}>{l}</Text>
      <Ionicons
      ...
      <View style={s.card}>
        ...
        <View style={s.tHead}>
          ...
          <SBtn
            c="barangay"
            l="Barangay"
            sty={{ flex: 1, paddingHorizontal: 10, paddingVertical: 8 }}
          />
          <SBtn
            c="count"
            l="Count"
...
export default function DashboardScreen({ navigation }) {
  ...
  const presetLbl =
    PRESETS.find((p) => p.key === applied.preset)?.label || "Custom";
  ...
  return (
    <SafeAreaView style={s.safe}>
        ...
        >
          ...
          >
            ...
            {applied.crimeTypes.map((c) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  handleApply({
                    ...applied,
                    crimeTypes: applied.crimeTypes.filter((x) => x !== c),
                  })
                }
              >
            ...
            {applied.barangays.map((b) => (
              <TouchableOpacity
                key={b}
                style={[s.aC, { backgroundColor: "#dcfce7" }]}
                onPress={() =>
                  handleApply({
                    ...applied,
                    barangays: applied.barangays.filter((x) => x !== b),
                  })
                }
              >
                <Text style={[s.aCTxt, { color: GREEND }]} numberOfLines={1}>
                  {b}
                </Text>
                <Text style={[s.aCTxt, { color: GREEND, opacity: 0.5 }]}>
                  ×
                </Text>
              </TouchableOpacity>
            ))}
      ...
      {loading ? (
        ...
        <ScrollView
          style={{ flex: 1, backgroundColor: G50 }}
          contentContainerStyle={{ padding: 12, paddingBottom: 90 }}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
              refreshing={refreshing}
              onRefresh={() => doFetch(applied, true)}
              colors={[NAVY]}
              tintColor={NAVY}
            />
          }
