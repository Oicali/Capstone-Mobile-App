...
const otp = StyleSheet.create({
  row:    { flexDirection: 'row', gap: 8, justifyContent: 'center', marginVertical: 22 },
...
function PasswordInput({ value, onChangeText, placeholder, hasError, editable = true }) {
  const [show, setShow] = useState(false);
  return (
    <View style={[pi.row, hasError && pi.rowErr]}>
      <Ionicons name="lock-closed-outline" size={17} color={C.textMuted} style={{ marginRight: 4 }} />
      <TextInput
        style={pi.input}
      ...
      <TouchableOpacity onPress={() => setShow(v => !v)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
        ...
        <Ionicons name={show ? 'eye-outline' : 'eye-off-outline'} size={19} color={C.textMuted} />
      </TouchableOpacity>
    </View>
  );
}
const pi = StyleSheet.create({
  row:    { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: C.border, borderRadius: 12, backgroundColor: C.surfaceAlt, paddingHorizontal: 12, gap: 4 },
...
function StatusScreen({ iconBg, iconColor, iconName, title, message, submessage, onClose, btnColor }) {
  return (
    <View style={lk.wrap}>
      ...
      <TouchableOpacity style={[lk.btn, { backgroundColor: btnColor || C.navy }]} onPress={onClose} activeOpacity={0.8}>
...
const lk = StyleSheet.create({
  ...
  iconCircle:     { width: 96, height: 96, borderRadius: 48, alignItems: 'center', justifyContent: 'center', marginBottom: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.1, shadowRadius: 14, elevation: 6 },
  title:          { fontSize: 20, fontWeight: '800', color: C.text, marginBottom: 12, textAlign: 'center', letterSpacing: -0.3 },
  msg:            { fontSize: 14, color: C.textSub, textAlign: 'center', lineHeight: 22, paddingHorizontal: 8, marginBottom: 6 },
  ...
  btn:            { marginTop: 20, paddingVertical: 15, borderRadius: 13, alignItems: 'center', width: '100%', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 10, elevation: 4 },
...
export default function ChangePasswordScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const activeTs = step === 'blocked'        ? blockedUntilTs
                   : step === 'session-locked' ? sessionLockedUntilTs
                   : step === 'pw-locked'      ? pwLockedUntilTs
                   : null;
    if (!activeTs) return;
    const tick = () => {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      if      (d.blocked)      { setBlockedHours(d.hoursLeft ?? 0); setBlockedUntilTs(Date.now() + (d.msLeft ?? (d.hoursLeft ?? 24) * 3_600_000)); setStep('blocked'); }
      else if (d.sessionLocked){ const lm = d.minsLeft ?? 15; setSessionLockMins(lm); setSessionLockedUntilTs(Date.now() + lm * 60_000); await AsyncStorage.setItem('cpm_session_locked', JSON.stringify({ until: Date.now() + lm * 60_000 })); setStep('session-locked'); }
      else if (d.pwLocked)     { setPwLockedMins(d.minsLeft ?? 15); setPwLockedUntilTs(Date.now() + (d.minsLeft ?? 15) * 60_000); setStep('pw-locked'); }
    ...
    const tick = () => {
      ...
      if (secs <= 0) {
        ...
        if (resendsLeftRef.current <= 0) {
          ...
          const lockMins = 15;
          AsyncStorage.setItem('cpm_session_locked', JSON.stringify({ until: Date.now() + lockMins * 60_000 })).catch(() => {});
  ...
  const handleVerifyCurrent = async () => {
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) {
        if (d.locked || d.pwLocked) { const lm = d.minutesLeft ?? 15; setPwLockedMins(lm); setPwLockedUntilTs(Date.now() + lm * 60_000); setStep('pw-locked'); return; }
        if (d.sessionLocked) {
          const lm = d.minutesLeft ?? 15; setSessionLockMins(lm); setSessionLockedUntilTs(Date.now() + lm * 60_000);
  ...
  const handleRequestOtp = async () => {
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) {
        if (d.blocked)       { setBlockedHours(d.hoursLeft ?? 0); setBlockedUntilTs(Date.now() + (d.msLeft ?? (d.hoursLeft ?? 24) * 3_600_000)); setStep('blocked'); return; }
        if (d.sessionLocked) {
          const lm = d.minutesLeft ?? 15; setSessionLockMins(lm); setSessionLockedUntilTs(Date.now() + lm * 60_000);
          await AsyncStorage.setItem('cpm_session_locked', JSON.stringify({ until: Date.now() + lm * 60_000 }));
          setStep('session-locked'); return;
        ...
        setFormErrors({ general: d.message || 'Failed to send code' }); return;
      }
      setOtpMasked(d.maskedEmail || '');
      const rl = d.resendsLeft ?? 2; setResendsLeft(rl); resendsLeftRef.current = rl;
      setOtpValues(['','','','','','']);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (d.blocked) { setBlockedHours(d.hoursLeft ?? 0); setBlockedUntilTs(Date.now() + (d.msLeft ?? (d.hoursLeft ?? 24) * 3_600_000)); setStep('blocked'); return; }
        if (d.resendsLeft === 0) { setResendsLeft(0); resendsLeftRef.current = 0; setOtpError('No more resends available.'); return; }
        setOtpError(d.message || 'Failed to resend code'); return;
      }
      const rl = d.resendsLeft ?? 0; setResendsLeft(rl); resendsLeftRef.current = rl;
      setOtpState('active');
      if (d.otpExpiresAt) startOtpTimer(d.otpExpiresAt);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (d.forceResend || d.attemptLocked || res.status === 429) {
          ...
          clearInterval(otpTimerRef.current);
          setOtpTimer(0);
          if (d.resendsLeft !== undefined) { setResendsLeft(d.resendsLeft); resendsLeftRef.current = d.resendsLeft; }
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          {step === 'blocked' && (
            <View style={lk.wrap}>
              ...
              <Text style={lk.msg}>
                You've already changed your password <Text style={{fontWeight:'800'}}>twice</Text> in the last 24 hours.{'\n\n'}
                This limit protects your account from unauthorized changes. 
              </Text>
              <Text style={[lk.msg, {fontWeight:'700', color: C.text, marginTop: 4}]}>You can change your password again in:</Text>
              <View style={lk.countdownBadge}>
                <Ionicons name="time-outline" size={16} color= '#C1272D' />
                <Text style={lk.countdownTxt}>{blockedCountdown || 'Calculating…'}</Text>
              </View>
              <TouchableOpacity style={[lk.btn, { backgroundColor:     '#0B2D6B', }]} onPress={() => navigation.goBack()} activeOpacity={0.8}>
          ...
          {step === 'session-locked' && (
            <View style={lk.wrap}>
              ...
              <Text style={[lk.msg, {fontWeight:'700', color: C.text, marginTop: 4}]}>Try again in:</Text>
              <View style={[lk.countdownBadge, { backgroundColor: C.amberLight, borderColor: '#FCD34D' }]}>
                <Ionicons name="time-outline" size={16} color={C.amber} />
                <Text style={[lk.countdownTxt, { color: C.amber }]}>{sessionCountdown || 'Calculating…'}</Text>
              </View>
              <TouchableOpacity style={[lk.btn, { backgroundColor: C.amber }]} onPress={() => navigation.goBack()} activeOpacity={0.8}>
          ...
          {step === 'pw-locked' && (
            <View style={lk.wrap}>
              ...
              <Text style={lk.msg}>
                Your account is temporarily locked due to too many incorrect password entries.{'\n\n'}
                Please wait for the timer to expire before trying again.
              </Text>
              <Text style={[lk.msg, {fontWeight:'700', color: C.text, marginTop: 4}]}>Try again in:</Text>
              <View style={[lk.countdownBadge, { backgroundColor: C.amberLight, borderColor: '#FCD34D' }]}>
                <Ionicons name="time-outline" size={16} color={C.amber} />
                <Text style={[lk.countdownTxt, { color: C.amber }]}>{pwCountdown || 'Calculating…'}</Text>
              </View>
              <TouchableOpacity style={[lk.btn, { backgroundColor: C.amber }]} onPress={() => navigation.goBack()} activeOpacity={0.8}>
                <Text style={lk.btnTxt}>Got it, Go Back</Text>
              </TouchableOpacity>
          ...
          {step === 'done' && (
            <View style={s.doneWrap}>
              ...
              <Text style={s.doneSub}>Your password has been updated and all active sessions have been revoked.</Text>
              <Text style={s.doneSub}>A security notification was sent to your email.</Text>
          ...
          {step === 'verify-current' && (
            <View style={s.stepWrap}>
              ...
              <Text style={s.stepSub}>Enter your current password to continue.</Text>
              ...
              <View style={s.btnRow}>
                ...
                <TouchableOpacity style={s.outlineBtn} onPress={() => navigation.goBack()} disabled={curLoading}>
                  <Text style={s.outlineBtnTxt}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.primaryBtn, s.primaryBtnFlex, (!curPassword.trim() || curLoading) && s.btnDisabled]}
          ...
          {step === 'form' && (
            <View style={s.stepWrap}>
              ...
              <Text style={s.stepSub}>Create a strong password that meets all requirements below.</Text>
              ...
              <View style={s.fieldCard}>
                ...
                {formErrors.newPassword ? (
                  <View style={s.errRow}><Ionicons name="alert-circle-outline" size={13} color={C.danger}/><Text style={s.errTxt}>{formErrors.newPassword}</Text></View>
                ) : null}
                ...
                {formErrors.confirmPassword ? (
                  <View style={s.errRow}><Ionicons name="alert-circle-outline" size={13} color={C.danger}/><Text style={s.errTxt}>{formErrors.confirmPassword}</Text></View>
                ) : null}
              </View>
              ...
              <View style={s.reqCard}>
                <View style={s.reqCardHeader}>
                  <Ionicons name="checkmark-circle-outline" size={15} color={C.navy} />
                  <Text style={s.reqCardTitle}>PASSWORD REQUIREMENTS</Text>
                </View>
                {reqStatus.map(r => (
                  <View key={r.id} style={s.reqRow}>
                    <View style={[s.reqDot, r.met ? s.reqDotMet : newPassword.length > 0 ? s.reqDotFail : s.reqDotIdle]}>
                      <Ionicons name={r.met ? 'checkmark' : 'remove'} size={10} color={r.met ? C.white : newPassword.length > 0 ? C.white : C.textMuted} />
                    </View>
                    <Text style={[s.reqTxt, r.met ? s.reqTxtMet : newPassword.length > 0 ? s.reqTxtFail : {}]}>{r.label}</Text>
                  </View>
                ))}
                <View style={s.reqRow}>
                  <View style={[s.reqDot, passwordsMatch ? s.reqDotMet : confirmPassword.length > 0 ? s.reqDotFail : s.reqDotIdle]}>
                    <Ionicons name={passwordsMatch ? 'checkmark' : 'remove'} size={10}
                      color={passwordsMatch ? C.white : confirmPassword.length > 0 ? C.white : C.textMuted} />
                  </View>
                  <Text style={[s.reqTxt, passwordsMatch ? s.reqTxtMet : confirmPassword.length > 0 ? s.reqTxtFail : {}]}>
                    Passwords match
                  </Text>
                </View>
              </View>
              ...
              <View style={s.btnRow}>
                ...
                <TouchableOpacity style={s.outlineBtn} onPress={() => setStep('verify-current')} disabled={isLoading}>
                  <Text style={s.outlineBtnTxt}>Back</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.primaryBtn, s.primaryBtnFlex, (!allMet || !passwordsMatch || isLoading) && s.btnDisabled]}
                  onPress={handleRequestOtp}
                  disabled={isLoading || !allMet || !passwordsMatch}>
                  {isLoading ? <ActivityIndicator size="small" color={C.white} /> : null}
                  {/* FIX: removed send icon from Send Code button */}
                  <Text style={s.primaryBtnTxt}>{isLoading ? 'Sending Code…' : 'Send Code'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
          ...
          {step === 'otp' && (
            <View style={s.stepWrap}>
              ...
              <View style={s.infoCard}>
                <View style={s.infoCardIcon}><Ionicons name="mail-open-outline" size={18} color="#1d4ed8" /></View>
                <View style={{ flex: 1 }}>
                  <Text style={s.infoCardTitle}>Code sent to <Text style={{ fontWeight: '700', color: C.navy }}>{otpMasked}</Text></Text>
                  <Text style={s.infoCardSub}>Expires in <Text style={{ fontWeight: '700' }}>2 minutes</Text>. Do not share.</Text>
                </View>
              </View>
              ...
              {otpState !== 'attempts-exceeded' && (
                <View style={[
                  ...
                  <Text style={[s.timerTxt, otpTimer <= 30 && otpTimer > 0 && { color: C.amber }, otpTimer === 0 && { color: C.danger }]}>
              ...
              {otpError !== '' && (
                <View style={[s.banner, otpState === 'attempts-exceeded' && s.bannerAmber]}>
                  <Ionicons name={otpState === 'attempts-exceeded' ? 'warning' : 'close-circle'} size={16} color={C.white} />
                  <Text style={s.bannerTxt}>{otpError}</Text>
                </View>
              )}

              <OtpRow values={otpValues}
                onChange={(idx, val) => setOtpValues(prev => { const n = [...prev]; n[idx] = val; return n; })}
                disabled={isLoading || otpState !== 'active'} />

              {/* FIX: "Sending..." / Confirm button is always visible and solid */}
              {otpState === 'active' && (
                <TouchableOpacity
                  style={[s.primaryBtn, s.primaryBtnFull, (otpValues.join('').length !== 6 || isLoading) && s.btnDisabled]}
                  onPress={handleVerifyOtp}
                  disabled={otpValues.join('').length !== 6 || isLoading}>
                  {isLoading && <ActivityIndicator size="small" color={C.white} />}
                  <Text style={s.primaryBtnTxt}>{isLoading ? 'Verifying…' : 'Confirm Password Change'}</Text>
                </TouchableOpacity>
              )}
              ...
              <View style={s.resendWrap}>
                {resendsLeft <= 0 ? (
                  ...
                  <TouchableOpacity style={[s.resendBtn, isLoading && { opacity: 0.5 }]} onPress={handleResend} disabled={isLoading}>
                    <Ionicons name="refresh" size={14} color={isLoading ? C.textLight : C.navy} />
                    <Text style={[s.resendTxt, isLoading && { color: C.textLight }]}>
...
const s = StyleSheet.create({
  ...
  backBtn:      { width: 44 },
  backBtnInner: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center' },
  ...
  doneIconCircle: { width: 88, height: 88, borderRadius: 44, backgroundColor: C.green, alignItems: 'center', justifyContent: 'center', marginBottom: 22, shadowColor: C.green, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 10 },
  doneTitle:      { fontSize: 22, fontWeight: '800', color: C.green, marginBottom: 14, textAlign: 'center', letterSpacing: -0.4 },
  doneSub:        { fontSize: 14, color: C.textSub, textAlign: 'center', lineHeight: 22, marginBottom: 6, paddingHorizontal: 8 },
  doneSpinRow:    { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 20, backgroundColor: '#ECFDF5', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 20 },
  ...
  stepIconCircle: { width: 64, height: 64, borderRadius: 32, backgroundColor: C.navyLight, alignItems: 'center', justifyContent: 'center', shadowColor: C.navy, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 3 },
  stepTitle:      { fontSize: 20, fontWeight: '800', color: C.text, textAlign: 'center', marginBottom: 8, letterSpacing: -0.3 },
  stepSub:        { fontSize: 13, color: C.textSub, textAlign: 'center', lineHeight: 20, marginBottom: 22, paddingHorizontal: 8 },

  fieldCard:   { backgroundColor: C.surface, borderRadius: 18, padding: 20, marginBottom: 16, shadowColor: C.navy, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 12, elevation: 3, borderWidth: 1, borderColor: C.border },
  fieldLabel:  { fontSize: 10, fontWeight: '800', color: C.textMuted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 },
  inputRow:    { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: C.border, borderRadius: 12, backgroundColor: C.surfaceAlt, paddingHorizontal: 12, gap: 8 },
  ...
  matchRow: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 9 },
  matchDot: { width: 18, height: 18, borderRadius: 9, alignItems: 'center', justifyContent: 'center' },
  matchTxt: { fontSize: 12, fontWeight: '600' },

  reqCard: { backgroundColor: C.surface, borderRadius: 18, padding: 18, marginBottom: 20, borderLeftWidth: 4, borderLeftColor: C.navy, shadowColor: C.navy, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2, borderWidth: 1, borderColor: C.border },
  reqCardHeader: { flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 14 },
  reqCardTitle:  { fontSize: 10, fontWeight: '800', color: C.navy, textTransform: 'uppercase', letterSpacing: 0.8 },
  reqRow:        { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 9 },
  reqDot:        { width: 20, height: 20, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: C.border },
  ...
  outlineBtn:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 14, paddingHorizontal: 18, borderRadius: 13, borderWidth: 1.5, borderColor: C.border, backgroundColor: C.surface },
  outlineBtnTxt: { fontSize: 14, fontWeight: '700', color: C.textSub },
  primaryBtn:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 15, paddingHorizontal: 20, borderRadius: 13, backgroundColor: C.navy, shadowColor: C.navy, shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 5 },
  ...
  banner:      { flexDirection: 'row', backgroundColor: C.danger, borderRadius: 12, padding: 13, alignItems: 'flex-start', marginBottom: 14, gap: 9 },
  bannerAmber: { backgroundColor: C.amber },
  bannerTxt:   { color: C.white, fontSize: 13, fontWeight: '600', flex: 1, lineHeight: 19 },

  infoCard:     { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#EFF6FF', borderRadius: 14, padding: 14, marginBottom: 12, gap: 12, borderWidth: 1, borderColor: '#BFDBFE' },
  infoCardIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: C.white, alignItems: 'center', justifyContent: 'center' },
  ...
  timerPill:   { flexDirection: 'row', alignItems: 'center', gap: 7, backgroundColor: C.navyLight, borderRadius: 10, paddingVertical: 10, paddingHorizontal: 14, marginBottom: 10, justifyContent: 'center' },
